...
struct GisConverter {
    ...
    static func convert4326To3857(longitude: Double, latitude: Double) -> (x: Double, y: Double) {
        let x = longitude.degreesToRadians * radius
        let y = log(tan(Double.pi / 4 + (latitude.degreesToRadians / 2))) * radius
    ...
    static func convert3857To4326(x: Double, y: Double) -> (longitude: Double, latitude: Double) {
        ...
        let lat4326 = (atan(exp(y / radius)) * 2 - Double.pi/2).radiansToDegrees
